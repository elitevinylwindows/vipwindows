...
class InstallerJobController extends Controller
{
    ...
    public function reschedule(Request $request, $id)
    {
        ...
        // Installer only flags the job with a reason — admin assigns the new date
        $job->update([
            'reschedule_reason' => $validated['reason'],
            'rescheduled_at'    => now(),
        ]);

        return response()->json(['success' => true, 'message' => 'Reschedule request submitted. Admin will assign a new date.']);
    }
    ...
    public function rescheduleEvent(Request $request, $id)
    {
        ...
        // Installer only flags the event with a reason — admin assigns the new date
        $event->update([
            'reschedule_reason' => $validated['reason'],
            'rescheduled_at'    => now(),
