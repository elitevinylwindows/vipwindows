...
@push('styles')
<style>
    .sj-container { display: flex; height: calc(100vh - 130px); overflow: hidden; }
    ...
    .sj-rail-header { padding: 1.25rem 1rem .75rem; border-bottom: 1px solid rgba(0,0,0,.06); }
    .sj-rail-header h6 { font-size: .75rem; text-transform: uppercase; letter-spacing: 1.2px; color: rgba(0,0,0,.4); margin-bottom: .75rem; }
    .sj-rail-search { display: flex; gap: .5rem; }
    .sj-rail-search input {
        flex: 1; background: #f5f5f5; border: 1px solid rgba(0,0,0,.1);
        color: #111; border-radius: .375rem; padding: .4rem .75rem; font-size: .85rem;
    }
    .sj-rail-search input::placeholder { color: rgba(0,0,0,.35); }
    ...
    .sj-rail-tabs .tab-btn.active { background: var(--vip-primary); color: #fff; border-color: var(--vip-primary); }
    ...
    .sj-card:hover { background: #fafafa; border-color: rgba(201,168,76,.3); }
    .sj-card.active { background: rgba(201,168,76,.06); border-color: var(--vip-accent); }
    .sj-card .sj-name { font-weight: 600; font-size: .9rem; color: #111; }
    .sj-card .sj-addr { font-size: .78rem; color: rgba(0,0,0,.5); margin-top: 2px; }
    .sj-card .sj-meta { display: flex; justify-content: space-between; align-items: center; margin-top: .35rem; }
    .sj-card .sj-date { font-size: .7rem; color: rgba(0,0,0,.4); }
    .sj-card .sj-badge { font-size: .6rem; padding: 2px 6px; border-radius: 3px; font-weight: 600; text-transform: uppercase; }
    .sj-badge-scheduled { background: rgba(0,123,255,.1); color: #0d6efd; }
    .sj-badge-completed { background: rgba(40,167,69,.1); color: #198754; }
    .sj-badge-rescheduled { background: rgba(108,117,125,.1); color: #6c757d; }

    .sj-rail-footer {
        padding: .75rem 1rem; border-top: 1px solid rgba(0,0,0,.06);
        font-size: .75rem; color: rgba(0,0,0,.4);
    }
    ...
    @media (max-width: 991.98px) {
        .sj-container { flex-direction: column; height: auto; min-height: calc(100vh - 130px); }
        ...
        <div class="sj-rail-list" id="sjRailList">
            @forelse($events as $e)
                <div class="sj-card" data-id="{{ $e->id }}" onclick="loadEvent({{ $e->id }}, this)">
                    ...
                    @if($e->crew)
                        <div class="sj-addr mt-1"><i class="bi bi-people me-1"></i>{{ $e->crew->name }}</div>
                    @endif
                </div>
            @empty
...
function renderDetail(data) {
    ...
    // Toolbar — admin sees status badges + time logged
    let actions = '';
    if (e.total_time_minutes > 0) {
        const hrs = Math.floor(e.total_time_minutes / 60), mins = e.total_time_minutes % 60;
        actions += `<span class="badge bg-info text-white me-2" style="font-size:.75rem;"><i class="bi bi-clock me-1"></i>Total: ${hrs}h ${mins}m</span>`;
    }
    actions += `<span class="badge bg-${e.event_status === 'completed' ? 'success' : e.event_status === 'rescheduled' ? 'secondary' : 'primary'}" style="font-size:.75rem;">${escHtml(e.event_status?.replace('_',' ') || 'Scheduled')}</span>`;
    ...
    let html = `
        <div class="sj-info-grid">
            ...
            <div class="sj-info-card"><div class="label">Email</div><div class="value">${e.customer_email ? `<a href="mailto:${e.customer_email}">${escHtml(e.customer_email)}</a>` : '—'}</div></div>
    ...
    if (job) {
        ...
        // Time logs for admin
        if (job.time_logs && job.time_logs.length) {
            html += `<h6 class="section-title"><i class="bi bi-clock-history"></i> Time Logs</h6>
                <div class="sj-checklist">
                    <table class="table table-sm mb-0">
                        <thead><tr><th>Team Member</th><th>Clock In</th><th>Clock Out</th><th>Duration</th></tr></thead>
                        <tbody>`;
            job.time_logs.forEach(log => {
                const hrs = log.total_minutes ? Math.floor(log.total_minutes / 60) : 0;
                const mins = log.total_minutes ? log.total_minutes % 60 : 0;
                html += `<tr>
                    <td>${escHtml(log.user_name)}</td>
                    <td>${escHtml(log.clock_in || '—')}</td>
                    <td>${escHtml(log.clock_out || 'Active')}</td>
                    <td>${log.total_minutes ? hrs + 'h ' + mins + 'm' : '—'}</td>
                </tr>`;
            });
