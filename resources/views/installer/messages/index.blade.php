...
<style>
    ...
    .msg-search { padding: .5rem 1rem; border-bottom: 1px solid rgba(255,255,255,.06); }
    .msg-search input {
    ...
    /* Compose bar */
    .msg-compose { background: #fff; border-top: 1px solid rgba(0,0,0,.08); padding: .75rem 1.5rem; }
    ...
    /* Recording indicator */
    .recording-bar { display: none; align-items: center; gap: .75rem; padding: .5rem 0; }
    ...
    /* Attachment preview */
    .attach-preview { display: none; padding: .5rem 0; gap: .5rem; align-items: center; }
        ...
        <div class="msg-list">
            ...
            @empty
                <div class="text-center py-5" style="color:rgba(255,255,255,.35);">
                    ...
                    <p class="small" style="color:rgba(255,255,255,.3);">Click "New" to start a conversation</p>
                </div>
            @endforelse
        </div>
    </div>
...
const basePath = '/installer/messages';
...
function loadConversation(id) {
    ...
    .then(r => r.json())
    .then(data => {
        document.getElementById('chatTitle').textContent = data.conversation.admin?.name || 'Admin';
...
function playVoice(btn, url) {
    ...
    audio.addEventListener('error', () => {
        ...
        audio.remove();
        // Fallback: try opening the file directly so the browser's native player handles it
...
async function toggleRecording() {
    ...
    try {
        ...
        // Pick a supported audio format (webm preferred, fallback to ogg, then mp4, then default)
        let mimeType = '';
