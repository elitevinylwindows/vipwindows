...
@section('breadcrumb')
<li class="breadcrumb-item"><a href="{{ route('admin.quotes.index') }}">{{ __('Quotes') }}</a></li>
<li class="breadcrumb-item active" aria-current="page">{{ __('Create Quote') }}</li>
@endsection
        ...
        <div class="col-12">
            <div class="card shadow">
                ...
                <div class="card-body py-2">
                    <form id="quoteHeaderForm" action="{{ $quote ? route('admin.quotes.update', $quote->id) : route('admin.quotes.store') }}" method="POST">
                        @csrf
                        @if($quote)
...
<div class="col-md-2 d-flex">
    <div class="card shadow w-100 d-flex flex-column">
        <form id="quoteItemForm" method="POST" action="{{ $quote ? route('admin.quotes.storeItem', $quote->id) : '#' }}" class="d-flex flex-column flex-grow-1" style="min-height:0;">
            @csrf
...
(function(){
    ...
    function loadShapeData() {
        ...
        fetch("{{ route('admin.quotes.shapes') }}")
            .then(r => r.json())
            .then(data => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    (function loadSeriesNfrc() {
        fetch('{{ route("admin.quotes.seriesMap") }}', { credentials: 'same-origin' })
    ...
    function calcPrice() {
        ...
        fetch('/admin/quotes/check-price', {
            method: 'POST',
    ...
    function handleDelete(e) {
        ...
        fetch(`/admin/quotes/${quoteId}/items/${itemId}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } })
        .then(r => r.json())
        .then(d => {
            if (d.success) {
    ...
    function fetchPanelLayout(seriesType) {
        ...
        let url = `/admin/quotes/panel-layout?series_type=${encodeURIComponent(seriesType)}`;
        if (W > 0) url += `&width=${W}`;
        if (H > 0) url += `&height=${H}`;
    ...
    document.getElementById('saveDraftButton')?.addEventListener('click', function() {
        ...
        fetch(`/admin/quotes/${quoteId}/save-draft`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify(data) })
        .then(r => r.json())
        .then(d => {
            if (d.success) { if (typeof toastr !== 'undefined') toastr.success('Quote saved as draft successfully!'); else alert('Quote saved as draft successfully!'); setTimeout(() => { window.location.href = "{{ route('admin.quotes.index') }}"; }, 1000); }
        ...
        document.getElementById('confirmSubmitQuoteBtn')?.addEventListener('click', function() {
            ...
            .then(d => {
                if (d.success) {
                    ...
                    setTimeout(() => { window.location.href = "{{ route('admin.quotes.index') }}"; }, 1500);
                }
    ...
    (function() {
        ...
        document.getElementById('applyDiscountsBtn')?.addEventListener('click', function() {
            ...
            fetch('{{ $quote ? route("admin.quotes.applyDiscounts", $quote->id) : "#" }}', {
                method: 'POST',
