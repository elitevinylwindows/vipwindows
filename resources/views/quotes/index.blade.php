...
<div class="container-fluid py-4 px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold mb-0"><i class="bi bi-calculator me-2"></i>Quotes</h4>
    ...
    <div class="mb-4">
        <div class="btn-group btn-group-sm" role="group">
            <a href="{{ route('admin.quotes.index') }}" class="btn {{ $status === 'all' ? 'btn-dark' : 'btn-outline-dark' }}">All</a>
            <a href="{{ route('admin.quotes.index', ['status' => 'draft']) }}" class="btn {{ $status === 'draft' ? 'btn-dark' : 'btn-outline-dark' }}">Draft</a>
            <a href="{{ route('admin.quotes.index', ['status' => 'sent']) }}" class="btn {{ $status === 'sent' ? 'btn-dark' : 'btn-outline-dark' }}">Sent</a>
        </div>
    </div>
                            ...
                            @foreach($quotes as $quote)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        ...
                                        <form method="POST" action="{{ route('admin.quotes.destroy', $quote->id) }}" class="d-inline" onsubmit="return confirm('Delete this quote?')">
                                            @csrf @method('DELETE')
                                ...
                                <div class="modal fade" id="sendQuote{{ $quote->id }}" tabindex="-1">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header"><h5 class="modal-title">Send Quote #{{ $quote->quote_number }}</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
